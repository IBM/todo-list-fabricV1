					...
					try {
						...
						eventhub.registerTxEvent(request.txId.toString(), (tx, code) => {
							...
							if (code !== 'VALID') {
								if (cb && !cbCalled) {
									cbCalled = true;
									return cb(common.format_error_msg('Commit code: ' + code),null);//pass error back
								}
								else return;
							} else {
								if (cb && !cbCalled) {
									cbCalled = true;
									return cb(null,tx);					//all good, pass it back
								}
								else return;
							}
						});
						...
						if (cb && !cbCalled) {
							cbCalled = true;
							return cb(e,null);								//all terrible, pass it back
						}
						else return;
					}
